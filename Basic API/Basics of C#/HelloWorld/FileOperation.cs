                ...
                try
                {
                    switch (menu[input])
                    {
                        case "Create":
                            ...
                            {
                                byte[] info = new UTF8Encoding(true).GetBytes("Initial Content\n");
                                //fs.Write(info, 0, info.Length);
                            }
                            Console.WriteLine("File.Create executed: Created demo.txt with initial content.");
                            break;
                        ...
                        case "Open":
                            using (FileStream fs = File.Open(movedPath, FileMode.Open))
                            {
                                Console.WriteLine($"File.Open executed: Opened {movedPath}.");
                            }
                            break;

                        case "OpenHandle":
                            var handle = File.OpenHandle(movedPath);
                            Console.WriteLine($"File.OpenHandle executed: Obtained handle for {movedPath}.");
                            break;

                        case "OpenRead":
                            using (var reader = File.OpenRead(movedPath))
                            {
                                Console.WriteLine($"File.OpenRead executed: Opened {movedPath} for reading.");
                            }
                            break;

                        case "OpenText":
                            using (StreamReader sr = File.OpenText(movedPath))
                            {
                        ...
                        case "ReadAllBytes":
                            byte[] data = File.ReadAllBytes(movedPath);
                            Console.WriteLine($"File.ReadAllBytes executed: Read {data.Length} bytes from {movedPath}.");
                            break;

                        case "ReadAllLines":
                            string[] readLines = File.ReadAllLines(movedPath);
                            Console.WriteLine("File.ReadAllLines executed: Lines:");
                            foreach (var line in readLines) Console.WriteLine($" - {line}");
                            break;

                        case "ReadAllText":
                            string content = File.ReadAllText(movedPath);
                        ...
                        case "ReadLines":
                            ...
                            foreach (var line in File.ReadLines(movedPath))
